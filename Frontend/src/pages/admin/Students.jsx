            ...
            <Card key={row._id} className="p-5">
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3 min-w-0">
                  <div className="w-11 h-11 rounded-full bg-teal-500 text-white flex items-center justify-center font-semibold text-sm shrink-0">
                    {(row.user?.name || '?')
                      .split(' ')
                      .map((w) => w[0])
                      .slice(0, 2)
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-ink-500">
                      Roll No. {row.rollNumber} · {row.classRoom ? classLabel(row.classRoom) : '—'}
                    </p>
                  </div>
                </div>
                {canManage && (
              ...
              <div className="flex items-center justify-between mt-4 pt-4 border-t border-ink-100 text-xs">
                <div>
                  <p className="text-ink-500">Admission No</p>
                  <p className="text-ink-900 font-medium mt-0.5">{row.admissionNumber}</p>
                </div>
                <div className="text-right">
                  <p className="text-ink-500">Parent</p>
                  <p className="text-ink-900 font-medium mt-0.5">{row.parentName || '—'}</p>
                </div>
              </div>
              {row.parentPhone && (
                <p className="text-xs text-ink-500 mt-2">📞 {row.parentPhone}</p>
              )}
            </Card>
          ))}
        </div>
