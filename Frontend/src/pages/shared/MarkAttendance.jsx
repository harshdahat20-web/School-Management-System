...
export default function MarkAttendance() {
  ...
  function setStatus(student, status) {
    setRows((rs) => rs.map((r) => (r.student === student ? { ...r, status } : r)))
  }
  ...
  return (
    <div className="max-w-4xl">
      ...
      <p className="text-sm text-ink-500 mt-1">Mark attendance for your class.</p>
      ...
      <Card className="mt-6 p-5">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 items-end">
          ...
          <Button variant="ghost" onClick={handleLoad} disabled={loading || !selectedClass}>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {rows.length === 0 ? (
                  ...
                  rows.map((row) => (
                    <tr key={row.student} className="border-b border-ink-100 last:border-0">
                      ...
                      <td className="px-5 py-3">
                        <div className="flex justify-end">
                          <StatusPicker
                            value={row.status}
                            onChange={(status) => setStatus(row.student, status)}
                          />
                        </div>
                      </td>
                    </tr>
